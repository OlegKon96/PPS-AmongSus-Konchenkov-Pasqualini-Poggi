...
import it.amongsus.core.entities.player.{AlivePlayer, CrewmateAlive, CrewmateGhost, DeadPlayer, ImpostorAlive, ImpostorGhost, Player}
  ...
  private class GamePanelImpl(map: Array[Array[Tile]],
    ...
    private def drawMap(g: Graphics): Unit = {
      gameMap.foreach(x => x.foreach {
        case tile: Vent => g.drawImage(vent, tile.position.y * SCALEFACTOR + ONE,
          tile.position.x * SCALEFACTOR + ONE,
          SCALEFACTOR, SCALEFACTOR, this)
        case tile: Emergency => g.drawImage(emergency, tile.position.y * SCALEFACTOR + ONE,
          tile.position.x * SCALEFACTOR + ONE,
          SCALEFACTOR, SCALEFACTOR, this)
        case tile: Boundary => g.drawImage(space, tile.position.y * SCALEFACTOR + ONE,
          tile.position.x * SCALEFACTOR + ONE,
          SCALEFACTOR, SCALEFACTOR, this)
        case tile: Wall => g.drawImage(block, tile.position.y * SCALEFACTOR + ONE,
          tile.position.x * SCALEFACTOR + ONE,
          SCALEFACTOR, SCALEFACTOR, this)
        case tile: Floor => g.drawImage(floor, tile.position.y * SCALEFACTOR + ONE,
          tile.position.x * SCALEFACTOR + ONE,
          SCALEFACTOR, SCALEFACTOR, this)
        case tile: Other => g.drawImage(space, tile.position.y * SCALEFACTOR + ONE,
          tile.position.x * SCALEFACTOR + ONE,
          SCALEFACTOR, SCALEFACTOR, this)
      })
    }
      ...
      gameMyChar match {
        case _: ImpostorAlive => {
          gameDeadBodies.filter(body => body.position.distance(gameMyChar.position) < gameMyChar.fieldOfView)
            .foreach(deadBody => g.drawImage(getImageDead(deadBody.color), deadBody.position.y * SCALEFACTOR + ONE,
        ...
        case _: ImpostorGhost => {
          ...
          gamePlayers.foreach {
            ...
            case impostorGhost: ImpostorGhost => {
              drawImpostor(g, impostorGhost, getImageGhost(impostorGhost.color))
            }
            case crewmateAlive: CrewmateAlive => {
              drawCrewmate(g, crewmateAlive, getImageAlive(crewmateAlive.color))
            }
            case crewmateGhost: CrewmateGhost => {
              drawCrewmate(g, crewmateGhost, getImageGhost(crewmateGhost.color))
            }
          }
        ...
        case _: CrewmateGhost => {
          ...
          gamePlayers.foreach {
            case ap: AlivePlayer => {
              drawCrewmate(g, ap, getImageAlive(ap.color))
            }
            case dp: DeadPlayer => {
              drawCrewmate(g, dp, getImageGhost(dp.color))
            }
          }
